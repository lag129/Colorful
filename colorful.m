classdef mixjuce < audioPlugin
	...
	methods
		function plugin = mixjuce
			for i = 1:length(plugin.bands) %#ok
				plugin.octFilts{i} = octaveFilter('FilterOrder',plugin.N, ...
					'CenterFrequency',plugin.bands(i), 'Bandwidth',plugin.bandWidth, 'SampleRate',getSampleRate(plugin)); %#ok
            end
            plugin.objEQU1 = multibandParametricEQ('NumEQBands',3,'EQOrder',4, ...
			    ...
			    'Frequencies',[200, 1000, 10000],'PeakGains',ones(1,3)*0,'QualityFactors',ones(1,3)*1.6,'SampleRate',getSampleRate(plugin));
		end
		...
		function out = process(plugin, in)
			if plugin.power
				...
				parfor i = 1:length(plugin.bands)
					result = result + plugin.octFilts{i}(in); %#ok
				end		
        ...
        function reset(plugin)
            for i = 1:length(plugin.bands)
			    plugin.octFilts{i}.SampleRate = getSampleRate(plugin);
